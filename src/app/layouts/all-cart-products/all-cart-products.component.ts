...
export class AllCartProductsComponent  implements OnInit{
  ...
  userId:number=0
  productsData=[{id:0,quantity:0,title:'',price:0}]
...
onUpdateData(cartId: number, productId: number, quantity: number,title:string){
  this.showUpdateBtn=true

  this.apiService.getUpdateTheCart(cartId,[{id:productId,quanty:quantity,title:title}]).subscribe({next:(res)=>{
    this.fetchAllCartData()
  },error:(err)=>{console.log(err)}})
}
